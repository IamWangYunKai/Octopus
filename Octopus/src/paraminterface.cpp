...
bool ParamInterface::setData(const QModelIndex &index, const QVariant &value,int role){
    Q_UNUSED(role)
    if (!index.isValid()){
        qDebug() << "Error in ParamInterface::setData";
        return false;
    }
    TreeItem *item = static_cast<TreeItem*>(index.internalPointer());
    item->changeData(2,value.toString());
    item->changeData(1,judgeType(value));
    paramManager->changeParam(item->parentItem()->data(0),item->data(0),value);
...
void ParamInterface::setupModelData(){
    ...
    foreach(QString groupName, paramManager->allGroups()){
        QStringList value;
        value << groupName << " " << " ";
        auto* groupParent = new TreeItem(value,rootItem);
        rootItem->appendChild(groupParent);
        foreach(QString key,paramManager->allKeys(groupName)){
            auto&& temp = paramManager->value(groupName,key).toString();
            QStringList value;
